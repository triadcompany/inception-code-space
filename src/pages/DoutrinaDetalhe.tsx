...
const DoutrinaDetalhe = () => {
  const { id } = useParams<{ id: string }>();
  const [item, setItem] = useState<DoutrinaItem | null>(null);
  ...
  const handleShare = async () => {
    try {
      await navigator.share({ title: item?.titulo, url: window.location.href });
    } catch {
  ...
  const readingTime = item?.conteudo
    ? Math.max(1, Math.ceil(item.conteudo.split(/\s+/).length / 200))
    : null;
      ...
      <main className="flex-1">
        <section className="relative pt-24 pb-14 overflow-hidden">
          ...
          <div className="container mx-auto max-w-3xl px-4 relative z-10">
            <Link to="/doutrina" className="inline-flex items-center gap-2 text-sm text-[hsl(215,20%,70%)] hover:text-white transition-colors mb-6">
              <ArrowLeft className="w-4 h-4" /> Voltar à Doutrina
            </Link>
            {loading ? (
              <div className="h-20" />
            ) : item ? (
              <div className="animate-fade-in-up">
                <div className="inline-flex items-center gap-2 bg-[hsl(var(--primary)/0.15)] border border-[hsl(var(--primary)/0.3)] rounded-full px-3 py-1 mb-4">
                  ...
                  <span className="text-xs font-medium text-[hsl(var(--primary))]">Doutrina</span>
                </div>
          ...
          <div className="container mx-auto max-w-3xl">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-sm text-[hsl(220,15%,55%)]">Carregando...</p>
              </div>
            ) : !item ? (
              <div className="text-center py-20">
                ...
                <Link to="/doutrina" className="text-[hsl(var(--primary))] text-sm mt-2 inline-block hover:underline">
                  Voltar à doutrina
                </Link>
              </div>
            ) : (
              <article className="animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
                {item.resumo && (
                  <div className="bg-[hsl(var(--primary)/0.06)] border-l-4 border-[hsl(var(--primary))] rounded-r-xl p-5 mb-8">
                    <div className="prose prose-sm max-w-none text-[hsl(220,20%,35%)] leading-relaxed italic prose-p:my-1" dangerouslySetInnerHTML={{ __html: item.resumo }} />
                  </div>
                )}
                {item.conteudo && (
                  <div className="prose prose-lg max-w-none text-[hsl(220,20%,25%)] leading-[1.85] prose-headings:font-display prose-headings:text-[hsl(220,30%,20%)] prose-a:text-[hsl(var(--primary))] prose-strong:text-[hsl(220,30%,20%)]" dangerouslySetInnerHTML={{ __html: item.conteudo }} />
                )}
                <div className="mt-12 pt-8 border-t border-[hsl(220,20%,92%)] flex items-center justify-between">
                  <Link to="/doutrina" className="inline-flex items-center gap-2 text-sm text-[hsl(220,15%,50%)] hover:text-[hsl(var(--primary))] transition-colors">
                    <ArrowLeft className="w-4 h-4" /> Outros estudos
                  </Link>
                  <Button variant="outline" size="sm" onClick={handleShare} className="border-[hsl(var(--primary))] text-[hsl(var(--primary))] hover:bg-[hsl(var(--primary))] hover:text-white transition-colors">
...
export default DoutrinaDetalhe;
